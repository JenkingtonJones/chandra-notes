...
// Azure LLM API URL - configured via env so the private endpoint is not
// hardcoded in source; falls back to a local default.
const AZURE_LLM_API_URL = process.env.AZURE_LLM_API_URL || "http://localhost:5000/chat";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Azure OpenAI streaming endpoint
  app.get("/api/azure-openai/generate-stream", async (req, res) => {
    try {
      // Check for large headers early to prevent HTTP 431 errors
      const headerSize = JSON.stringify(req.headers).length;
      if (headerSize > 8192) { // 8KB header limit
        console.warn(`Large headers detected on Azure request: ${headerSize} bytes`);
        return res.status(431).json({ error: "Request headers too large" });
      }

      const { prompt, system = "", model = "gpt-4o", temperature = 0.7, max_tokens = 2000, endpoint, apiKey } = req.query;
      ...
      if (!endpoint || !apiKey) {
        return res.status(400).json({ error: "Azure endpoint and API key are required" });
      }

      console.log("Starting Azure OpenAI streaming request with model:", model);
      ...
      // Reasoning models (o1, o3 series) don't accept temperature parameter
      const o1Models = ['o1-pro', 'o1', 'o1-mini'];
      const isO1Model = o1Models.includes(model as string);
      ...
      const stream: any = await azureOpenai.chat.completions.create(requestParams);
      ...
      res.status(500).json({
        status: "error",
        message: error instanceof Error ? error.message : "Failed to stream Azure OpenAI response"
      });
    }
  });

  // Azure LLM diagnostic endpoint to test token limits
  app.get("/api/azure/test-limits", async (req, res) => {
    try {
      const testPrompt = "Return exactly 1000 words of text explaining various programming concepts";
      
      console.log("Testing Azure LLM API with different token limits...");
      
      const results = [];
      
      // Test with different token limits
      for (const tokenLimit of [500, 1000, 2000, 4000]) {
        console.log(`Testing with max_tokens=${tokenLimit}`);
        
        try {
          const response = await axios.post(AZURE_LLM_API_URL, 
            { prompt: testPrompt, max_tokens: tokenLimit, temperature: 0.7 }, 
            { headers: { 'Content-Type': 'application/json' } }
          );
          
          const responseLength = response.data.response ? response.data.response.length : 0;
          console.log(`Response length with max_tokens=${tokenLimit}: ${responseLength}`);
          
          results.push({
            tokenLimit,
            responseLength,
            truncated: responseLength > 0 && !response.data.response.endsWith('.') 
              && !response.data.response.endsWith('!') 
              && !response.data.response.endsWith('?')
          });
        } catch (error) {
          console.error(`Error testing with max_tokens=${tokenLimit}:`, error);
          results.push({
            tokenLimit,
            error: true,
      ...
      console.error("Azure LLM API diagnostic error:", error);
      res.status(500).json({
        status: "error",
        message: error instanceof Error ? error.message : "Error running diagnostic"
      });
    }
  });
  ...
  app.post("/api/azure/chat", async (req, res) => {
    try {
      ...
      console.log(`Sending request to Azure LLM API with prompt: ${prompt.substring(0, 100)}...`);
      console.log(`Azure LLM API URL: ${AZURE_LLM_API_URL}`);
      
      // For diagnostic purposes, let's make sequential requests with increasing token limits
      let responseResult = null;
      let fullText = "";
      const maxRequests = 3; // Make up to 3 sequential requests for very long content
      
      // Try with different token limits to see if we can get a complete response
      for (let i = 0; i < maxRequests; i++) {
        // We'll try with a token limit of 1000 for each request, starting where we left off
        const continuePrompt = i === 0 ? prompt : `${prompt}\n\n[Continue from here]\n${fullText}`;
        
        console.log(`Making request ${i+1}/${maxRequests} with token limit 1000...`);
        ...
        // Save the response on first iteration
        if (i === 0) {
        ...
        if (response.data.response) {
          ...
          console.log(`Request ${i+1} response length: ${responseText.length}`);
          console.log(`Last few chars: ${responseText.slice(-20)}`);
          
          // Add to our accumulated text
          fullText += responseText;
          
          // If the response ends with proper punctuation or is short enough, assume we're done
          if (responseText.length < 800 || 
              responseText.endsWith('.') || 
              responseText.endsWith('!') || 
              responseText.endsWith('?')) {
            console.log(`Response appears complete after ${i+1} requests.`);
            break;
          }
        } else {
          console.log(`Request ${i+1} returned no response text.`);
          break;
        }
      }
      
      console.log(`Total accumulated response length: ${fullText.length}`);
      
      // Return the full accumulated response if we made multiple requests
      if (fullText.length > 0 && fullText !== responseResult.response) {
        res.json({
          response: fullText,
          original_response: responseResult.response,
          combined_from_multiple_requests: true
        });
      } else {
        res.json(responseResult);
      }
    } catch (error) {
      console.error("Azure LLM API error:", error);
      ...
      if (axios.isAxiosError(error)) {
        ...
        res.status(500).json({
          status: "error",
          message: error instanceof Error ? error.message : "Failed to generate completion from Azure LLM API"
        });
      }
    }
  });
